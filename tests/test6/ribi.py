...
if __name__ == '__main__':
    ...
    num_trees = int(input())
    kriterija = input()
    zapis = [float(el) for el in input().split(' ')]

    #ispechati accuracy i klasa

    encoder = OrdinalEncoder()
    encoder.fit([row[:-1] for row in dataset])

    train_set = dataset[:int(0.85 * len(dataset))]  # slicing nema da raboti bez int, gi zimame prvite x%
    ...
    test_set = dataset[int(0.85 * len(dataset)):]  # gi zimame poslednite (100-x)%
    ...
    train_x_2 = list()
    for t in train_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]  # se trga most important, analogno kje trgnesh i za ispiti
    ...
    classifier = RandomForestClassifier(n_estimators=num_trees, criterion=kriterija)  # 3 drva na odluka
    ...
    tmp = zapis.pop(col_index)
    check = classifier.predict([zapis])
    print(check[0])
