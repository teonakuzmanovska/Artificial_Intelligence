...
def move_forward(green, shx, shy, s_body, dir):
    s_body = list(s_body)
    green = list(green)

    if dir == "north":
        if shy + 1 <= 9 and (shx, shy + 1) not in s_body:
        ...
        if shy - 1 >= 0 and (shx, shy - 1) not in s_body:
            if (shx, shy - 1) not in green:
                s_body.pop()  # go vadam posledniot element od teloto
            else:
                green.remove((shx, shy - 1))
        ...
        if shx - 1 >= 0 and (shx - 1, shy) not in s_body:
        ...
        if shx + 1 <= 9 and (shx + 1, shy) not in s_body:
            if (shx + 1, shy) not in green:
                s_body.pop()  # go vadam posledniot element od teloto
            else:
                green.remove((shx + 1, shy))
...
def move_right(green, shx, shy, s_body, dir):
...
def move_left(green, shx, shy, s_body, dir):
    s_body = list(s_body)
    green = list(green)
    if dir == "east":
        if shy + 1 <= 9 and (shx, shy + 1) not in s_body:
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        # snake = Snake((green_list, snake_body, direction))
        green_app = state[0]
        ...
        tmpx, tmpy, tmpdir, tmp_body, tmp_green = move_forward(green_app, sh_x, sh_y, s_body, direct)
        if (tmpx, tmpy, tmpdir) != (sh_x, sh_y, direct):
            successors["ProdolzhiPravo"] = (tmp_green, tmp_body, tmpdir)

        tmpx, tmpy, tmpdir, tmp_body, tmp_green = move_right(green_app, sh_x, sh_y, s_body, direct)
        if (tmpx, tmpy, tmpdir) != (sh_x, sh_y, direct):
            successors["SvrtiDesno"] = (tmp_green, tmp_body, tmpdir)

        tmpx, tmpy, tmpdir, tmp_body, tmp_green = move_left(green_app, sh_x, sh_y, s_body, direct)
        if (tmpx, tmpy, tmpdir) != (sh_x, sh_y, direct):
    ...
    def h(self, node):
        # print(len(node.state[0]))
        # return len(node.state[0])
        if len(node.state[0]) > 0:
            return len(node.state[0])
        else:
            return 0
...
if __name__ == '__main__':
    ...
    snake_body = ((0, 7), (0, 8), (0, 9))  # vaka dobivam 16.67/50 zatoa shto e razlichna mojata od nivnata hevristika
    direction = "south"
    ...
    # result = greedy_best_first_graph_search(snake)
